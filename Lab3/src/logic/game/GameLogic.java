...
public class GameLogic {
    ...
    public boolean isHandPlayable(int playerIndex) {
        // TODO Implement here
    	for (BaseCard card : this.getPlayerHand(playerIndex)) {
    ...
    public ArrayList<BaseCard> draw(int amount) {
        // TODO Implement here
    	int realAmount = Math.min(amount,deck.size());
    	ArrayList<BaseCard> cardToDraw = new ArrayList<>();
    	cardToDraw.addAll(deck.subList(0, realAmount));
    	deck.subList(0, realAmount).clear();
    	this.getCurrentPlayerHand().addAll(cardToDraw);
    	return cardToDraw;
    }
